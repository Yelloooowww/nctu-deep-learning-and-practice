    ...
    def evaluate(engine):
        if args.dataset == "task1":
            ...
            with torch.no_grad():
                ...
                predict_x = model(y_onehot=test_conditions, temperature=1, reverse=True).float()
                score = evaluator.eval(predict_x, test_conditions)
                save_image(postprocess(predict_x).float(), args.output_dir+f"/Epoch{engine.state.epoch}_score{score:.3f}.png",normalize=True)

                test_conditions = get_new_test_conditions(args.dataroot).cuda()
                predict_x = model(y_onehot=test_conditions, temperature=1, reverse=True).float()
                newscore = evaluator.eval(predict_x.float(), test_conditions)
                save_image(postprocess(predict_x).float(), args.output_dir+f"/Epoch{engine.state.epoch}_newscore{newscore:.3f}.png",normalize=True)
