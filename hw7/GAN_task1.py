...
if __name__=='__main__':
    ...
    netG = _netG(ngpu=1, nz=nz).to(device) #nz: size of the latent z vector
    ...
    for epoch in range(1,epochs+1):
        for i, (images,conditions) in enumerate(loader_train):
            ...
            if score > best_score :
                ...
                best_score = score
                # savefig
                save_image(gen_imgs, os.path.join('results', 'epoch'+str(epoch)+'_iter_'+ str(iter)+'_'+str(score)+'.png'), nrow=8, normalize=True)
            ...
            writer.add_scalars("cGAN_task1_Train", {    "errD_fake":errD_fake.item(),\
                                                        ...
                                                        "score": score  }, iter)
            writer.flush()
            if i % 10 == 0: print(epoch,i,errD_fake.item(),errD_real.item(),errG.data.item(),'score = ',score)
