...
class EncoderRNN(nn.Module):
	def __init__(
		...
		self.lstm = nn.LSTM(input_size=hidden_size,hidden_size=hidden_size,dropout=0.7)
	...
	def forward(self, inputs, c):
		...
		out, (h_n, c_n) = self.lstm(input_seq,(hidden,hidden))
		...
		z = (torch.rand(1,1,32).to(device)) * torch.exp(logvar/2) + mean # Latent torch.Size([1, 1, 32])
...
def evaluation(encoder, decoder, dataset,show=True):
	...
	for idx in range(len(dataset)):
		...
		z, mean, logvar = encoder(inputs, target_condition)
	...
	if show:
		...
		for j in range(10):
			...
			noise = torch.rand(1,1,32).to(device)
...
if __name__=='__main__':
	...
	for epoch in range(1, epochs+1):
		...
		if epoch < epochs*0.5 : tfr = 0.95
		else : tfr = max(0.95-(epoch-epochs*0.5)/(epochs*0.5), 0)
